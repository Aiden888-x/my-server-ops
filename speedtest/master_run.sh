...
do
    ...
    if [ $? -eq 0 ]; then
        # 【最稳逻辑】
        # 1. awk -F': ' '{print $2}' 从冒号处切开，取后面部分
        # 2. awk '{print $1, $2}' 只取切开后的前两个单词（数字+单位）
        PING=$(echo "$RESULT" | grep "Latency" | awk -F': ' '{print $2}' | awk '{print $1, $2}')
